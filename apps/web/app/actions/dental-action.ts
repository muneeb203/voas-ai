...
export async function updateStaffHoursAction(staffId: string, body: StaffHoursInput) {
  const { error, session } = await requireOwner('/staff');
  if (error || !session) return { error: error ?? 'Unauthorized' };
  const res = await updateStaffHours(session.active.workspace_id, staffId, body);
  if (isApiError(res)) return { error: res.error.message };
  revalidatePath('/staff');
  return { error: null };
}
