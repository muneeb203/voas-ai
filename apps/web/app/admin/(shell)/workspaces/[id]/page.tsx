...
export default async function AdminWorkspaceDetailPage({
  ...
  // Only the workspace detail blocks the initial paint — one fast call. Everything
  // else (tickets, usage, activity, kiosk metrics, KB, …) streams in behind
  // Suspense, so the page + tab bar open immediately instead of waiting on the
  // slowest of a dozen queries.
  const detailRes = await getAdminWorkspace(params.id);
...
async function WorkspaceTabs({
  ...
  const [
    ...
    getAdminWorkspaceUsage(id),
    listAdminWorkspaceGrants(id),
    getAdminKioskSettings(id),
    getAdminKioskMetrics(id),
    listAdminWorkspaceActivity(id),
  ...
  const logFilter = (['config', 'operation', 'error'] as const).includes(
    logParam as 'config' | 'operation' | 'error',
  )
    ? (logParam as 'config' | 'operation' | 'error')
    : 'all';
  const visibleLog =
      ...
      <TabsContent value="log">
        <div className="mb-4 flex items-center gap-1 border-b border-border">
          {LOG_FILTERS.map((f) => (
            <Link
              key={f.id}
              href={`/admin/workspaces/${id}?tab=log${f.id === 'all' ? '' : `&log=${f.id}`}`}
              className={cn(
