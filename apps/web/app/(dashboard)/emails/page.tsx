...
import { useWorkspaceContext } from '@/lib/auth/workspace';
...
export default function EmailsPage() {
  ...
  async function fetchSettings() {
    try {
      const res = await fetch(`/api/workspaces/${workspaceId}/email-settings`);
  ...
  async function fetchLogs() {
    try {
      const res = await fetch(`/api/workspaces/${workspaceId}/email-logs?limit=50`);
      const data = await res.json();
      setLogs(data.data || []);
    } catch (err) {
                ...
                {logs.map((log) => (
                  <tr key={log.id} className="border-b hover:bg-muted/50">
                    ...
                    <td className="px-6 py-3 text-sm">
                      {Math.floor(log.call_data.duration_seconds / 60)}m {log.call_data.duration_seconds % 60}s
                    </td>
                    <td className="px-6 py-3 text-sm">
